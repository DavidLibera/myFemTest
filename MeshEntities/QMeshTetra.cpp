...
UINT QMeshTetra::GetIndexNo(){
	return indexno;
}

void QMeshTetra::SetIndexNo( const UINT _index ){
	indexno = _index;
}
...
QMeshFace * QMeshTetra::GetFaceRecordPtr( const UINT whichFace ){
	return faces[whichFace-1];
}

void QMeshTetra::SetFaceRecordPtr( const UINT whichFace, QMeshFace * _face ){
	faces[whichFace-1] = _face;
}

QMeshEdge * QMeshTetra::GetEdgeRecordPtr( const UINT whichEdge ){
	switch (whichEdge){
...
void QMeshTetra::GetNodePos( const UINT whichNode, double &xx, double &yy, double &zz){
	this->GetNodeRecordPtr(whichNode)->GetCoord3D(xx, yy, zz);
}

QMeshNode * QMeshTetra::GetNodeRecordPtr( UINT whichNode ){
	switch (whichNode){
		case 1: case 2: case 3:
		...
		case 4:
			for (int i=1; i<=3; i++){
				bool exist = false;
				for (int j=1; j<=3; j++)
					if (faces[1]->GetNodeRecordPtr(i) == this->GetNodeRecordPtr(j))
						exist=true;
				if (!exist) return faces[1]->GetNodeRecordPtr(i);
			}
		default:
...
BOOL QMeshTetra::IsNormalDirection( const UINT whichFace )
{
	//ASSERT( (whichEdge==1) || (whichEdge==2) || (whichEdge==3) );
	UINT bitNumber = whichFace - 1;
	return isNormal[bitNumber];
}

void QMeshTetra::SetDirectionFlag( const UINT whichEdge, const BOOL toBe )
{
	//ASSERT( (whichEdge==1) || (whichEdge==2) || (whichEdge==3) );
...
double QMeshTetra::CalVolume(){
	double t[4][3];
	for (int i=0; i<4; i++)
		GetNodePos(i, t[i][0], t[i][1], t[i][2]);
