...
void GLKObList::RemoveAll()
{
	...
	nCount=0;	headPos=NULL;	tailPos=NULL; //weight=0;
}

GLKObject* GLKObList::GetNext( GLKPOSITION& rPosition )
{
	//if (rPosition->prev==tailPos) return NULL;
	GLKPOSITION tempObj=rPosition;
	rPosition=tempObj->next;
	return tempObj->data;
}

GLKObject* GLKObList::GetNext( GLKPOSITION& rPosition, bool direction)
{
	
	GLKPOSITION tempObj=rPosition;
	if (direction)
		rPosition=tempObj->next;
	else rPosition=tempObj->prev;
	return tempObj->data;
}
...
void GLKObList::Inverse(){
	GLKPOSITION tmp, process=headPos;
	while (process){
		tmp = process->next; process->next=process->prev; process->prev=tmp;
		process=process->prev;
	}
	tmp=headPos; headPos=tailPos; tailPos=tmp;
}
